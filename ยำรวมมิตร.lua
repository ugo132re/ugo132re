...
-- แก้ไขปุ่มเพื่อป้องกัน Error nil value
Tabs.Main:AddButton({ 
...
Tabs.Main:AddButton({ 
    Title = "Dark Dex V3", 
    Callback = function() 
        pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/Babyhamsta/RBLX_Scripts/main/Universal/BypassedDarkDexV3.lua"))() end)
    end 
})
...
game:GetService("RunService").RenderStepped:Connect(function()
    if AimConfig.Enabled then
        ...
        if Target then
            Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, Target.Character.Head.Position), AimConfig.Smoothness)
        end
    end
end)
